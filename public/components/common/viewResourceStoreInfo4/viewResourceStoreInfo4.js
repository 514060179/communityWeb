...
(function (vc) {
    vc.extends({
        ...
        _initMethod: function () {
            //根据请求参数查询 查询 业主信息
            $that._loadResourceStoreInfoData();
        ...
        _initEvent: function () {
            vc.on('viewResourceStoreInfo4', 'setResourcesOut', function (_resOrderType) {
                $that.viewResourceStoreInfo4.resOrderType = _resOrderType;
            });
            vc.on('viewResourceStoreInfo4', 'chooseResourceStore', function (_app) {
                vc.copyObject(_app, $that.viewResourceStoreInfo4);
                vc.emit($props.callBackListener, $props.callBackFunction, $that.viewResourceStoreInfo4);
            });
            vc.on('viewResourceStoreInfo4', 'onIndex', function (_index) {
                $that.viewResourceStoreInfo4.index = _index;
            });
            vc.on('viewResourceStoreInfo4', 'setSelectResourceStores', function (resourceStores) {
                let oldList = $that.viewResourceStoreInfo4.resourceStores;
                // 过滤重复选择的商品
                ...
                $that.viewResourceStoreInfo4.resourceStores = resourceStores;
            });
        ...
        methods: {
            _loadResourceSuppliers() {
                ...
                vc.http.apiGet('resourceSupplier.listResourceSuppliers',
                    ...
                    function (json, res) {
                        ...
                        $that.viewResourceStoreInfo4.resourceSuppliers = _resourceSupplierManageInfo.data;
                    },
            ...
            _listAllocationStorehouse: function (_page, _rows) {
                ...
                vc.http.apiGet('resourceStore.listStorehouses',
                    ...
                    function (json, res) {
                        ...
                        $that.viewResourceStoreInfo4.storehouses = _storehouseManageInfo.data;
                    },
                    function (errInfo, error) {
                        console.log('请求失败处理');
                    }
            ...
            storeHousesChange: function (e, i) {
                ...
                $that.viewResourceStoreInfo4.storehouses.forEach((item) => {
                    if (item.shId == shId) {
                        $that.viewResourceStoreInfo4.resourceStores[i].shzName = item.shName;
                    }
                })
            },
            // 移除选中item
            _removeSelectResourceStoreItem: function (resId) {
                $that.viewResourceStoreInfo4.resourceStores.forEach((item, index) => {
                    if (item.resId == resId) {
                        $that.viewResourceStoreInfo4.resourceStores.splice(index, 1);
                    }
                })
                // 同时移除子页面复选框选项
