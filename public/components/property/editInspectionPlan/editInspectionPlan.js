    ...
    vc.extends({
        ...
        _initMethod: function () {
            ...
            vc.getDict('inspection_plan', "sign_type", function (_data) {
                $that.editInspectionPlanInfo.signTypes = _data;
            });
            vc.getDict('inspection_plan', "inspection_plan_period", function (_data) {
                $that.editInspectionPlanInfo.inspectionPlanPeriods = _data;
            });
        },
        _initEvent: function () {
            // $that._initEditInspectionPlanDateInfo();
            vc.on('editInspectionPlan', 'openEditInspectionPlanModal', function (_params) {
                $that.refreshEditInspectionPlanInfo();
                vc.copyObject(_params, $that.editInspectionPlanInfo);
                $that.editInspectionPlanInfo.communityId = vc.getCurrentCommunity().communityId;
            ...
            vc.on('editInspectionPlanInfo', 'notify', function (_param) {
                $that.editInspectionPlanInfo.inspectionRouteId = _param.inspectionRouteId;
            })
        },
        methods: {
            _initInspectionPlanEditInfo: function () {
                ...
                $('.editInspectionPlanStartDate').datetimepicker()
                    .on('changeDate', function (ev) {
                        ...
                        var end = Date.parse(new Date($that.editInspectionPlanInfo.endDate));
                        if (start - end >= 0) {
                            vc.toast("巡检开始日期必须小于巡检结束日期");
                            $(".editInspectionPlanStartDate").val('');
                            $that.editInspectionPlanInfo.startDate = "";
                        } else {
                            $that.editInspectionPlanInfo.startDate = value;
                        }
                    });
                ...
                $('.editInspectionPlanEndDate').datetimepicker()
                    .on('changeDate', function (ev) {
                        ...
                        var start = Date.parse(new Date($that.editInspectionPlanInfo.startDate));
                        var end = Date.parse(new Date(value));
                        if (start - end >= 0) {
                            vc.toast("巡检结束日期必须大于巡检开始日期");
                            $(".editInspectionPlanEndDate").val('');
                            $that.editInspectionPlanInfo.endDate = "";
                        } else {
                            $that.editInspectionPlanInfo.endDate = value;
                        }
                    });
                ...
                $('.editInspectionPlanStartTime').datetimepicker()
                    .on('changeDate', function (ev) {
                        ...
                        // var start = Date.parse(new Date(value));
                        // var end = Date.parse(new Date($that.editInspectionPlanInfo.endTime));
                        // if (start - end >= 0) {
                        //     vc.toast("巡检开始时间必须小于巡检结束时间");
                        //     $(".editInspectionPlanStartTime").val('');
                        //     $that.editInspectionPlanInfo.startTime = "";
                        // } else {
                            $that.editInspectionPlanInfo.startTime = value;
                        // }
                    });
                ...
                $('.editInspectionPlanEndTime').datetimepicker()
                    .on('changeDate', function (ev) {
                        ...
                        // var start = Date.parse(new Date($that.editInspectionPlanInfo.startTime));
                        // var end = Date.parse(new Date(value));
                        // if (start - end >= 0) {
                        //     vc.toast("巡检结束时间必须大于巡检开始时间");
                        //     $(".editInspectionPlanEndTime").val('');
                        //     $that.editInspectionPlanInfo.endTime = "";
                        // } else {
                            $that.editInspectionPlanInfo.endTime = value;
                        // }
                    });
                //防止多次点击时间插件失去焦点
            ...
            editInspectionPlanValidate: function () {
                return vc.validate.validate({
                    editInspectionPlanInfo: $that.editInspectionPlanInfo
                }, {
            ...
            editInspectionPlan: function () {
                if (!$that.editInspectionPlanValidate()) {
                ...
                vc.http.apiPost(
                    ...
                    JSON.stringify($that.editInspectionPlanInfo),
                    {
                        emulateJSON: true
                    },
                    function (json, res) {
            ...
            refreshEditInspectionPlanInfo: function () {
                var signTypes = $that.editInspectionPlanInfo.signTypes;
                var inspectionPlanPeriods = $that.editInspectionPlanInfo.inspectionPlanPeriods;
                $that.editInspectionPlanInfo = {
                    inspectionPlanId: '',
                    inspectionPlanName: '',
                    inspectionRouteId: '',
                    inspectionPlanPeriod: '',
